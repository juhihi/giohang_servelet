					...
					<div class="account pull-right">
						<ul class="user-menu">
							<li><a href="<c:url value='/cart2?action=view' />">Your
									Cart 2 <b>(<span id="countTotal">${sessionScope.cart.tongsl}</span>)
								</b>
							</a></li>
				...
				<div class="row">
					<div class="span9">
						<div class="row">
							<div class="span4">
								<a
									href="<c:url value='home/themes/images/product/${product.hinh}' />"
									class="thumbnail" data-fancybox-group="group1"
									title="Description 1"> <img alt=""
									src="<c:url value='home/themes/images/hinhanh/${product.hinh}' />">
								</a>
							</div>
							<div class="span5">
								<address>
									<strong>Availability:</strong> <span>${product.soluong}</span><br>
									<input type="hidden" id="maSP" value="${product.masp}" /> <input
										type="hidden" id="soLuongTrongKho" value="${product.soluong}" />
								</address>
								<h4>
									<strong>Giá: <fmt:formatNumber type="number"
											maxFractionDigits="3" value="${product.giasp}" /></strong>
								</h4>
							</div>
							<div class="span5">
								<form class="form-inline">
									<p>&nbsp;</p>
									<label>Qty:</label> <input id="soLuongMua" type="number"
										value="1" class="span1" placeholder="1" min="1"
										max="${product.soluong}">
									<button class="btn btn-info" type="button" id="addToCart">Add
										to cart</button>
								</form>
							</div>
						</div>
	...
	$('#addToCart').click(
			function() {
				...
				if (soLuongMua > soLuongTrongKho || soLuongMua < 1) {
					...
					$.ajax(
							{
								url : 'cart2?action=add&masp=' + maSP
										+ '&soluong=' + soLuongMua
										+ '&isUpdate=0',
								dataType : 'json'
							}).then(function(data) {
